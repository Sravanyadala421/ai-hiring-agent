#!/usr/bin/env python3
"""
Streamlit web application for the Hiring Agent.
"""
        ...
        if uploaded_file is not None:
            ...
            st.success(f"✅ Uploaded: {uploaded_file.name}")
                ...
                if analyze_clicked or demo_clicked:
                    # Check for API issues first
                    llm_provider = os.getenv('LLM_PROVIDER', 'ollama')
                    if llm_provider == 'gemini' and analyze_clicked:
                        api_key = os.getenv('GEMINI_API_KEY', '')
                        if not api_key or api_key == 'your_gemini_api_key_here':
                    ...
                    if evaluation_result:
                        ...
                        st.error("❌ Analysis failed. Please check the console output below.")
